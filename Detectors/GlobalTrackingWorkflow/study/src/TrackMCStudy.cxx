    ...
    for (int row = 0; row < 152; row++) {
      ...
      for (unsigned int icl0 = 0; icl0 < TPCClusterIdxStruct.nClusters[sector][row]; icl0++) {
        ...
        for (const auto& lbl : labels) {
          ...
          if (mctr.minTPCRowSect == sector && mctr.maxTPCRowInner <= row) {
            mctr.minTPCRowSect = row;
          }
          if (ncontLb > 1) {
            mctr.nTPCClShared++;
          }
          ...
          if (params.minTPCRefsToExtractClRes > 0) {
            ...
            float occ = occBin < 0 ? mTBinClOcc[0] : (occBin >= mTBinClOcc.size() ? mTBinClOcc.back() : mTBinClOcc[occBin]);
            int nmeas = 0;
            float yRef = 0, zRef = 0, ey = 0, ez = 0, snp = 0, tgl = 0;
            if (okBelow) {
              yRef += tparBelow.getY();
              zRef += tparBelow.getZ();
              snp = tparBelow.getSnp();
              tgl = tparBelow.getTgl();
              nmeas++;
            }
            if (okAbove) {
              yRef += tparAbove.getY();
              zRef += tparAbove.getZ();
              snp = tparAbove.getSnp();
              tgl = tparBelow.getTgl();
              nmeas++;
            }
            if (nmeas) {
              if (nmeas > 1) {
                ...
                snp *= 0.5;
                tgl *= 0.5;
                ey = tparBelow.getY() - tparAbove.getY();
                ez = tparBelow.getZ() - tparAbove.getZ();
              }
              if (std::abs(ey) < params.maxTRefExtrapErr && std::abs(ez) < params.maxTRefExtrapErr) {
                (*mDBGOut) << "clres" << "sect=" << sector << "row=" << row << "x=" << xc << "y=" << yc << "z=" << zc << "yRef=" << yRef << "zRef=" << zRef << "ey=" << ey << "ez=" << ez
                           << "snp=" << snp << "tgl=" << tgl << "qmax=" << clus.getQmax() << "qtot=" << clus.getQtot() << "flags=" << clus.getFlags() << "ncont=" << ncontLb
                           << "ne=" << nmeas << "occ=" << occ << "\n";
              }
            }
          }
