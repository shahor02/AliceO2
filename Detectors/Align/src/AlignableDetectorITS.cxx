...
void AlignableDetectorITS::defineVolumes()
{
  ...
  AlignableVolume *volITS = nullptr, *volLr = nullptr, *volSt = nullptr, *volHSt = nullptr, *volMod = nullptr;
  ...
  for (int ilr = 0; ilr < geom->getNumberOfLayers(); ilr++) {
    ...
    for (int ist = 0; ist < geom->getNumberOfStaves(ilr); ist++) {
      addVolume(volSt = new AlignableVolume(geom->composeSymNameStave(ilr, ist), getNonSensLabel(nonSensCnt++), mController));
      sym2vol[volSt->getSymName()] = volSt;
      volSt->setParent(volLr);

      for (int ihst = 0; ihst < geom->getNumberOfHalfStaves(ilr); ihst++) {
        addVolume(volHSt = new AlignableVolume(geom->composeSymNameHalfStave(ilr, ist, ihst), getNonSensLabel(nonSensCnt++), mController));
        sym2vol[volHSt->getSymName()] = volHSt;
        volHSt->setParent(volSt);

        for (int imd = 0; imd < geom->getNumberOfModules(ilr); imd++) {
          addVolume(volMod = new AlignableVolume(geom->composeSymNameModule(ilr, ist, ihst, imd), getNonSensLabel(nonSensCnt++), mController));
  ...
  for (int ich = 0; ich < geom->getNumberOfChips(); ich++) {
    ...
    int lay = 0, sta = 0, ssta = 0, modd = 0, chip = 0;
    geom->getChipId(chID, lay, sta, ssta, modd, chip);
    AlignableVolume* parVol = sym2vol[modd < 0 ? geom->composeSymNameStave(lay, sta) : geom->composeSymNameModule(lay, sta, ssta, modd)];
    if (!parVol) {
...
//_________________________________________________
void AlignableDetectorITS::UpdatePointByTrackInfo(AlignmentPoint* pnt, const Track& t) const
{
