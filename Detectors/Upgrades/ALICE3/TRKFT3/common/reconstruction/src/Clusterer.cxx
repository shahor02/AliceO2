                                                                     ...
                                                                     float yPlaneMLOT) noexcept
{
  ...
  SegmentationChip::detectorToLocalUnchecked(cluster.row, cluster.col, x, z,
                                             cluster.subDetID, cluster.layer, cluster.layer);
...
//__________________________________________________
template <int DetID>
void Clusterer<DetID>::process(gsl::span<const Digit> digits,
...
//__________________________________________________
template <int DetID>
void Clusterer<DetID>::ClustererThread::initChip(gsl::span<const Digit> digits, uint32_t first, GeometryTGeo* geom)
{
...
//__________________________________________________
template <int DetID>
void Clusterer<DetID>::ClustererThread::updateChip(gsl::span<const Digit> digits, uint32_t ip)
{
                                            ...
                                            GeometryTGeo* geom)
{
  ...
  for (size_t i1 = 0; i1 < preClusterHeads.size(); ++i1) {
    ...
    if (bbox.isAcceptableSize()) {
      streamCluster(bbox, pixArrBuff, totalCharge, doLabels, nlab, chipID, subDetID, layer);
    } else {
      ...
      do {
        ...
        do {
          ...
          if (!subPix.empty()) {
            streamCluster(bboxT, subPix, subCharge, doLabels, nlab, chipID, subDetID, layer);
          }
...
template class Clusterer<o2::detectors::DetID::TRK>;
template class Clusterer<o2::detectors::DetID::FT3>;
