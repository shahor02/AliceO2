...
void Digitizer<DetID>::init()
{
  ...
  LOGP(info, "{} Digitizer is initialised.", o2::detectors::DetID::getName(DetID));
  mParams.print();
...
template <int DetID>
const o2::trkft3::ChipSimResponse* Digitizer<DetID>::getChipResponse(int chipID)
{
...
//_______________________________________________________________________
template <int DetID>
void Digitizer<DetID>::process(const std::vector<Hit>* hits, int evID, int srcID, int layer)
{
  ...
  for (int i : hitIdx | std::views::filter([&](int idx) {
                 ...
                 return getROFLayer((*hits)[idx].GetDetectorID()) == layer;
               })) {
...
//_______________________________________________________________________
template <int DetID>
void Digitizer<DetID>::setEventTime(const o2::InteractionTimeRecord& irt, int layer)
{
...
//_______________________________________________________________________
template <int DetID>
void Digitizer<DetID>::fillOutputContainer(uint32_t frameLast, int layer)
{
  ...
  o2::trkft3::ROFRecord rcROF; /// using temporarly trkft3::ROFRecord
  ...
  for (; mROFrameMin <= frameLast; mROFrameMin++) {
    ...
    for (auto& chip : mChips) {
      if (chip.isDisabled() || (layer >= 0 && getROFLayer(chip.getChipIndex()) != layer)) {
        continue;
      }
...
//_______________________________________________________________________
template <int DetID>
void Digitizer<DetID>::processHit(const o2::trkft3::Hit& hit, uint32_t& maxFr, int evID, int srcID, int rofLayer)
{
  ...
  const o2::trkft3::ChipSimResponse* resp = getChipResponse(chipID);
...
template class o2::trkft3::Digitizer<o2::detectors::DetID::TRK>;
template class o2::trkft3::Digitizer<o2::detectors::DetID::FT3>;
